...
// 6. Financial & Category Delivery Rules View Component
const FinancialView: React.FC = () => {
  const [categoryDeliveryRules, setCategoryDeliveryRules] = useState([
    { id: 'cat-1', name: 'المطاعم والوجبات السريعة', icon: '🍔', calcType: 'fixed', fixedFee: 1000, pricePerKm: 200, pricePerKg: 0, baseFee: 500 },
    { id: 'cat-2', name: 'الصيدليات والمستلزمات الطبية', icon: '💊', calcType: 'distance', fixedFee: 800, pricePerKm: 150, pricePerKg: 0, baseFee: 400 },
    { id: 'cat-3', name: 'السوبرماركت والتموينات', icon: '🛒', calcType: 'both', fixedFee: 1200, pricePerKm: 200, pricePerKg: 100, baseFee: 600 },
    { id: 'cat-4', name: 'الإلكترونيات والهواتف', icon: '📱', calcType: 'fixed', fixedFee: 1500, pricePerKm: 250, pricePerKg: 0, baseFee: 500 },
    { id: 'cat-5', name: 'الورود والهدايا', icon: '🌸', calcType: 'distance', fixedFee: 1000, pricePerKm: 300, pricePerKg: 0, baseFee: 500 },
    { id: 'cat-6', name: 'البهارات والمكسرات والقهوة', icon: '🌶️', calcType: 'weight', fixedFee: 900, pricePerKm: 0, pricePerKg: 200, baseFee: 400 },
    { id: 'cat-7', name: 'الحلويات والمخبوزات', icon: '🧁', calcType: 'fixed', fixedFee: 1000, pricePerKm: 200, pricePerKg: 0, baseFee: 500 },
    { id: 'cat-8', name: 'اللحوم والأسماك الطازجة', icon: '🥩', calcType: 'both', fixedFee: 1500, pricePerKm: 250, pricePerKg: 150, baseFee: 700 },
  ]);

  const [savedSuccessMsg, setSavedSuccessMsg] = useState<string | null>(null);

  const handleUpdateCategoryRule = (id: string, field: string, value: any) => {
    setCategoryDeliveryRules(prev => prev.map(rule => {
      if (rule.id === id) {
  ...
  const handleSaveAllDeliveryRules = () => {
    setSavedSuccessMsg('تم حفظ وتحديث قواعد حساب رسوم التوصيل المالية لجميع الفئات بنجاح في النظام!');
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">الإدارة المالية وتحديد رسوم التوصيل حسب الفئة</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              تحديد تكلفة التوصيل وطريقة الحساب (ثابتة، حسب المسافة، حسب وزن الطلب، أو كلاهما) لكل فئة نشاط
            </p>
          </div>
        </div>

        <button
          onClick={handleSaveAllDeliveryRules}
          className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs flex items-center gap-2 cursor-pointer shrink-0"
        >
          <CheckCircle2 className="w-4 h-4" />
          <span>حفظ إعدادات الرسوم المالية</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl space-y-1">
          <span className="text-xs font-bold text-blue-700">صافي عمولات المنصة والتوصيل</span>
        ...
        <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl space-y-1">
          <span className="text-xs font-bold text-amber-700">إجمالي الطلبات المسلمة</span>
          <div className="text-2xl font-bold text-amber-900 font-sans">1,420 طلب</div>
        </div>
      </div>

      {/* Delivery Fee Calculation Rules By Category */}
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-xs space-y-5">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div className="flex items-center gap-2">
            <Truck className="w-5 h-5 text-blue-600" />
            <h3 className="text-base font-bold text-slate-800">قواعد احتساب قيمة التوصيل لكل فئة (Category Delivery Matrix)</h3>
          </div>
          <span className="text-xs text-slate-400">حدد آلية الحساب والقيمة لكل نشاط تجاري</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {categoryDeliveryRules.map((cat) => (
            <div key={cat.id} className="p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3 hover:border-blue-300 transition-all">
              <div className="flex items-center justify-between border-b border-gray-200/80 pb-2">
                ...
                <span className="text-[10px] bg-blue-100 text-blue-800 font-bold px-2 py-0.5 rounded-full">
                  {cat.calcType === 'fixed' && 'قيمة ثابتة'}
                  {cat.calcType === 'distance' && 'حسب المسافة'}
                  {cat.calcType === 'weight' && 'حسب الوزن'}
                  {cat.calcType === 'both' && 'كلاهما (مسافة + وزن)'}
                </span>
              </div>
              ...
              <div className="space-y-3 pt-1">
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-700 block mb-1">
                    آلية الحساب المعتمدة لهذه الفئة:
                  </label>
                  <select
                    value={cat.calcType}
                    onChange={(e) => handleUpdateCategoryRule(cat.id, 'calcType', e.target.value)}
                    className="w-full px-3 py-1.5 rounded-lg border border-gray-300 text-xs font-bold bg-white text-slate-800 focus:ring-2 focus:ring-blue-500"
                  >
                    <option value="fixed">1. قيمة ثابتة (Fixed Fee)</option>
                    <option value="distance">2. على حسب المسافة (Distance-Based)</option>
                    <option value="weight">3. على حسب وزن الطلب (Weight-Based)</option>
                    <option value="both">4. كلاهما (المسافة + وزن الطلب)</option>
                  </select>
                </div>

                {/* Input parameters based on calculation mode */}
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {(cat.calcType === 'fixed' || cat.calcType === 'both') && (
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 block">القيمة الثابتة (ريال)</label>
                      <input
                        type="number"
                        value={cat.fixedFee}
                        onChange={(e) => handleUpdateCategoryRule(cat.id, 'fixedFee', Number(e.target.value))}
                        className="w-full px-2.5 py-1 rounded-md border border-gray-300 text-xs font-mono font-bold bg-white"
                        step="100"
                      />
                    </div>
                  )}

                  {(cat.calcType === 'distance' || cat.calcType === 'both') && (
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 block">سعر الكيلومتر (ريال/كم)</label>
                      <input
                        type="number"
                        value={cat.pricePerKm}
                        onChange={(e) => handleUpdateCategoryRule(cat.id, 'pricePerKm', Number(e.target.value))}
                        className="w-full px-2.5 py-1 rounded-md border border-gray-300 text-xs font-mono font-bold bg-white"
                        step="50"
                      />
                    </div>
                  ...
                  {(cat.calcType === 'weight' || cat.calcType === 'both') && (
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 block">سعر الكيلوجرام (ريال/كجم)</label>
                      <input
                        type="number"
                        value={cat.pricePerKg}
                        onChange={(e) => handleUpdateCategoryRule(cat.id, 'pricePerKg', Number(e.target.value))}
                        className="w-full px-2.5 py-1 rounded-md border border-gray-300 text-xs font-mono font-bold bg-white"
                        step="50"
                      />
                    </div>
                  ...
                  {cat.calcType !== 'fixed' && (
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 block">رسوم التأسيس (ريال)</label>
                      <input
                        type="number"
                        value={cat.baseFee}
                        onChange={(e) => handleUpdateCategoryRule(cat.id, 'baseFee', Number(e.target.value))}
                        className="w-full px-2.5 py-1 rounded-md border border-gray-300 text-xs font-mono font-bold bg-white"
                        step="100"
                      />
                    </div>
